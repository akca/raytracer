...
void read_jpeg(const char *filename, unsigned char *image, int width, int height) {
  ...
  jpeg_start_decompress(&cinfo);
  if ((unsigned int)width != cinfo.output_width ||
      (unsigned int)height != cinfo.output_height) {
    throw std::runtime_error(
        "Error: Actual JPEG resolution does not match the provided one.");
  }
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    for (j = 0; j < width; j++) {
      for (k = 0; k < 3; k++) {
        image[(cinfo.output_scanline - 1) * width * 3 + j * 3 + k] =
            (unsigned char)row_pointer[3 * j + k];
      }
    }
  }
...
void write_jpeg(const char *filename, unsigned char *image, int width, int height) {
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    ...
    for (j = 0; j < width; j++) {
      for (k = 0; k < 3; k++) {
        row_pointer[3 * j + k] =
            (JSAMPLE)image[cinfo.next_scanline * width * 3 + j * 3 + k];
      }
    }
